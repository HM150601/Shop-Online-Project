...
export const forget = (email, password) => async (dispatch) => {
  try {
    dispatch({ type: actions.USER_FORGET_REQUEST });
    ...
    dispatch({ type: actions.USER_FORGET_SUCCESS, payload: data.user });
    dispatch({ type: actions.USER_LOGIN_SUCCESS, payload: data.user });
    localStorage.setItem("userInfo", JSON.stringify(data.user));
  } catch (error) {
    dispatch({
      type: actions.USER_FORGET_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
